...
/**
@title Vault contract used for unit test
...
*/
contract VaultV2 is ERC1967UpgradeUpgradeable, UUPSUpgradeable {
    ...
    event ChequeBounced();
    event VaultWithdraw(address indexed from, uint256 amount);
    ...
    bytes32 public constant CHEQUE_TYPEHASH =
        keccak256("Cheque(address vault,address beneficiary,uint256 cumulativePayout)");
    ...
    function domain() internal view returns (EIP712Domain memory) {
        ...
        return EIP712Domain({name: "Vault", version: "1.0", chainId: chainId});
    }
    ...
    function domainSeparator(EIP712Domain memory eip712Domain) internal pure returns (bytes32) {
        return keccak256(abi.encode(EIP712DOMAIN_TYPEHASH, keccak256(bytes(eip712Domain.name)), keccak256(bytes(eip712Domain.version)), eip712Domain.chainId));
    }
    ...
    function recoverEIP712(bytes32 hash, bytes memory sig) internal view returns (address) {
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator(domain()), hash));
    ...
    */
    function _cashChequeInternal(
        ...
        if (msg.sender != issuer) {
            require(
                issuer == recoverEIP712(chequeHash(address(this), beneficiary, cumulativePayout), issuerSig),
        ...
        uint256 totalPayout = Math.min(requestPayout, totalbalance());
    ...
    */
    function cashChequeBeneficiary(address recipient, uint256 cumulativePayout, bytes memory issuerSig) public {
    ...
    function chequeHash(
        ...
        return keccak256(abi.encode(CHEQUE_TYPEHASH, vault, beneficiary, cumulativePayout));
    }
    ...
    function version() public pure returns (string memory v) {
        return "v2.0.0";
    }
}
