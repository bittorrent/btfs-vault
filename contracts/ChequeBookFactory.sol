...
contract ChequeBookFactory {
  ...
  event ChequeBookDeployed(address issuer,address contractAddress);
  ...
  */
  function deployChequeBook(address issuer, bytes32 salt)
  public returns (address) {    
    address contractAddress = Clones.cloneDeterministic(master, keccak256(abi.encode(msg.sender, salt)));
    ChequeBook(contractAddress).init(issuer, TokenAddress);
    deployedContracts[contractAddress] = true;
    emit ChequeBookDeployed(issuer,contractAddress);
